...
\begin{tikzpicture}[x=1cm,y=1cm]
    ...
    % \node at (9.2,-1.7) {\emph{(c)} Node-tree $\mathcal{N}$ (Node-Partitioning)};
    ...
    \begin{scope}[shift={(9.2,-1.7)}]
        \node at (0,0) {\emph{(c)} Node-tree $\mathcal{N}$ (Node-Partitioning)};
        \node[even1, pattern color=mgrey] (n) at (-2.6,.8){$N$};
        \node[parameters, anchor=west, align=left] (ni) at (-2,.8){boundary vertices, neighbors nodes,\\ $\rho_N$, $\pi_N$, $\tilde{\delta_N}$, $\omega_N$}; 
