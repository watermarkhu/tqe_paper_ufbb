...
\Figure[htb](topskip=0pt, botskip=0pt, midskip=0pt){figures/tikz-figure0.pdf}{
    A cluster with vertices $\{v_0, v_1, v_2\}$ with potential matching weights $\{2\frac{1}{2}, 3\frac{1}{2}, 2\frac{1}{2}\}$. The line style and color of the colored edges correspond to the matching in the hypothetical union with an external vertex $v'$ of the same line style and color.\label{fig0}}
...
\textcolor{cyan}{Since per growth iteration half-edges are attached to the odd cluster's boundary, the addition of $(v,w)$ to the cluster can either occur on the first half-edge, or the second. In the latter case other trivial vertices are also added to the cluster, which leads to an increased cluster size post growth. To distinguish between to two variants, we add $\frac{1}{2}$ to the PMW's of vertices with half-edges attached. Using this definition, the cluster with with vertices $\{v_0, v_1, v_2\}$ in \Cref{fig0} has PMW's $\{2\frac{1}{2}, 3\frac{1}{2}, 2\frac{1}{2}\}$.}
...
\textcolor{cyan}{The Node-Partitioning data structure does not replace but coexists with the Union-Find data structure, whose critical goal is to differentiate between clusters. In the original Union-Find decoder, a spanning-forest $F$ is created after cluster growth from the sub-graph $G_e$, comprised of all even parity clusters. In the improved algorithm, acyclic node-trees $\nset$ must be maintained during growth for recalculation after PMW changes, and spanning-trees $\vset$ must thus be maintained during growth by removing edges from a cluster if a cycle is detected. The resulting set of spanning-trees is equivalent to the spanning-forest $F$. % as it is con multiple breadth-first searches from each non-trivial vertex in the cluster within $G_e$. 
...
\begin{equation}
    \text{ND}(n) = 2\left(\text{PMW}(n) - \min_{x\in\nset}{\text{PMW}(x)}\right)    
\end{equation}
