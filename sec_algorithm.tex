...
If the subtree $\nset_Q$ is of odd parity, denoted by $\dot{\nset_Q}$, it must consist of a combination of even branches and an odd number of odd branches. By the same argument as before, the even branches must match internally. 
Moreover, For a hypothetical merger to even parity on either $Q$ or $P$, there must always be a matching between an odd branch within $\dot{\nset_Q}$ with either $Q$, $P$, or the next syndrome-node in $\nset_P$. 
