...
For this we introduce the \textbf{node-tree} data structure for a cluster $C$. \textcolor{red}{The following discussion assumes that clusters grow uniformly, i.e. when a cluster is grown all boundary edges are grown by half an edge.}

%Let the sub-graph $\vset\subseteq C$ be 
A spanning-tree of $C$ is a connected acyclic sub-graph that includes all vertices of $C$ and a minimum number of edges. 
A node-tree $\nset$ is a partition of the \textcolor{red}{leaves} of $\vset$, a spanning tree of $C$. Each element of $\nset$ --- which we call a \textbf{node} --- represents a set of 
boundary %???, discuss
vertices that all lie at the same distance --- the \textbf{node radius} $\rho$ --- from a \textbf{primer vertex} in $\vset$. In the following we discuss how the primer vertices are chosen.%, which initializes the node and lies at its center. 
%The node-tree is an undirected acyclic graph, and its edges have lengths equal to the distance (edges in $\vset$) between the primer vertices of neighboring nodes. 
...
First, we let every non-trivial vertex be the primer vertex of a node representing all the boundary vertices in $\vset$ that are closest to it, we call these nodes \textbf{syndrome-nodes}. 
We define a second type of nodes, the \textbf{junction-nodes}, their primer vertices are trivial vertices at equal distance from two or more non-trivial vertices.
% discuss, is this enough?
We denote syndrome and junction nodes by an upper case letter decorated with a dot, $\dot{N}$, or a bar, $\bar{N}$, respectively.
A cluster with a node-tree comprised of both node types is depicted in \Cref{fig:nodes}. 
...
Second, they avoid redundant computations since all of the vertices in each node have equal PMW, moreover, this succinct representation is easy to maintain:
 %However, or lie within the radius of another node. 
a growth iteration can be represented as an increase in the node radius by $\nicefrac{1}{2}$. 

For the example in \Cref{fig:pmw}, the PMW of all boundary vertices of $A$, henceforth just the PMW of $A$, is $\rho_A + \abs{(B,C)} +1$. %The partitioning of $\vset$ to $\nset$ thus allows us to compute the PMW using parameters of the reduced node-tree. 
...
\end{align*}
where $C$ is a constant equal to the weight of the matching with the subtrees $\dot{\nset_P}$ and $\overbar{\nset_Q}$. Filling these values into \Cref{eq:delayequation} we find that the difference in delay is 
\begin{equation*}
  \delta_{\dot{Q}} - \delta_{\dot{P}} = 2\left(\rho_{\dot{Q}} - \rho_{\dot{P}} - \abs{(\dot{P}, \dot{Q})}\right).
\end{equation*}

Similarly, the difference in node delay can be deduced for any combination of node types of $P,Q$ and parity of $\nset_Q$ (see \Cref{fig:nmcombi}):
\begin{align*}
