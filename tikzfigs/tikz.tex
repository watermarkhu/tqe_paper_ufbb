...
\begin{tikzpicture}[scale=.9, node distance=1cm, on grid]
    ...
    \begin{scope}[shift={(l)}]
        ...
        \path (0,0) node[subtree]{} -- +(.5,0)node[anchor=west]{Sub-tree};
    \end{scope}
    ...
    \node at (a0) {\footnotesize $\pre{k}\omega_2$};
    \node at (a1) {\footnotesize $\pre{k}\omega_1$};
    \node at (a2) {\footnotesize $\pre{k}\omega_0$};

    \foreach \i in {0,1,2}{
            \node (b\i) [above = \i cm of b] {};
        }
    \draw[enset] (b2) circle[radius=0.4cm];
    \node at (b2) {\footnotesize $\pre{k}\omega_0$};
    ...
    \node at ($(b0)!0.5!(b1)$) {\footnotesize $\pre{k}\epsilon$};
    ...
    \node at (c1) {\footnotesize $\pre{k-1}\omega$};
    ...
    \node (ca) at ($(c)!0.5!(a)$) {}; \node [below = 1cm of ca] {$f(\pre{k-1}\omega)$};
    \node (cb) at ($(c)!0.5!(b)$) {}; \node [below = 0.6cm of cb] {$f_\omega(\pre{k-1}\omega)$};
    \node [below = 0.6cm of b1t] {$f_\epsilon(\pre{k}\epsilon)$};
